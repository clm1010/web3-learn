...
console.log(ethers.formatUnits(oneGwei)); // 0.000000001 等同于formatUnits(value, "ether")
console.groupEnd();

// 3. 解析：大单位转小单位
// 例如将ether转换为wei：parseUnits(变量, 单位),parseUnits默认单位是 ether
console.group('\n3. 解析：大单位转小单位，parseUnits');
console.log(ethers.parseUnits("1.0").toString());           // 1000000000000000000
console.log(ethers.parseUnits("1.0", "ether").toString());  // 1000000000000000000
console.log(ethers.parseUnits("1.0", 18).toString());       // 1000000000000000000
console.log(ethers.parseUnits("1.0", "gwei").toString());   // 1000000000
console.log(ethers.parseUnits("1.0", 9).toString());        // 1000000000
console.log(ethers.parseUnits("1.0").toString());           // 1000000000000000000 等同于parseUnits(value, "ether")
console.groupEnd();