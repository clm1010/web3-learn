    ...
    AccessControlUpgradeable
{
    ...
    // ************************************** DATA STRUCTURE **************************************
    /*
    Basically, any point in time, the amount of CLMs entitled to a user but is pending to be distributed is:

    pending CLM = (user.stAmount * pool.accCLMPerST) - user.finishedCLM

    Whenever a user deposits or withdraws staking tokens to a pool. Here's what happens:
    1. The pool's `accCLMPerST` (and `lastRewardBlock`) gets updated.
    2. User receives the pending CLM sent to his/her address.
    3. User's `stAmount` gets updated.
    4. User's `finishedCLM` gets updated.
    */
    struct Pool {
        ...
        // Weight of pool
        uint256 poolWeight;
        // Last block number that CLMs distribution occurs for pool
        uint256 lastRewardBlock;
        // Accumulated CLMs per staking token of pool
        uint256 accCLMPerST;
    ...
    struct User {
        ...
        uint256 stAmount;
        // Finished distributed CLMs to user
        uint256 finishedCLM;
        // Pending to claim CLMs
        uint256 pendingCLM;
    ...
    // ************************************** STATE VARIABLES **************************************
    // First block that CLMStake will start from
    uint256 public startBlock;
    // First block that CLMStake will end from
    uint256 public endBlock;
    // CLM token reward per block
    uint256 public clmPerBlock;
    ...
    // ************************************** EVENT **************************************
    event SetCLM(IERC20 indexed CLM);
    ...
    event SetCLMPerBlock(uint256 indexed clmPerBlock);
    ...
    /**
     * @notice Set CLM token address. Set basic info when deploying.
     */
    ...
    function initialize(
        ...
        uint256 _startBlock,
        uint256 _endBlock,
        uint256 _clmPerBlock
    ) public initializer {
        require(
            _startBlock <= _endBlock && _clmPerBlock > 0,
            "invalid parameters"
        );
    ...
    /**
     * @notice Set CLM token address. Can only be called by admin
     */
    function setCLM(IERC20 _CLM) public onlyRole(ADMIN_ROLE) {
        CLM = _CLM;
    ...
    /**
     * @notice Update the CLM reward amount per block. Can only be called by admin.
     */
    function setCLMPerBlock(uint256 _clmPerBlock) public onlyRole(ADMIN_ROLE) {
        require(_clmPerBlock > 0, "invalid parameter");

        clmPerBlock = _clmPerBlock;

        emit SetCLMPerBlock(_clmPerBlock);
    }

    /**
     * @notice Add a new staking to pool. Can only be called by admin
     * DO NOT add the same staking token more than once. CLM rewards will be messed up if you do
     */
    function addPool(
        ...
        returns (uint256 multiplier)
    {
        ...
        bool success;
        (success, multiplier) = (_to - _from).tryMul(clmPerBlock);
        require(success, "multiplier overflow");
    }

    /**
     * @notice Get pending CLM amount of user in pool
     */
    function pendingCLM(uint256 _pid, address _user)
        external
        view
        checkPid(_pid)
        returns (uint256)
    {
        return pendingCLMByBlockNumber(_pid, _user, block.number);
    }

    /**
     * @notice Get pending CLM amount of user by block number in pool
     */
    function pendingCLMByBlockNumber(
        uint256 _pid,
        address _user,
        ...
        uint256 accCLMPerST = pool_.accCLMPerST;
        ...
        if (_blockNumber > pool_.lastRewardBlock && stSupply != 0) {
            ...
            uint256 clmForPool = (multiplier * pool_.poolWeight) /
                totalPoolWeight;
            accCLMPerST = accCLMPerST + (clmForPool * (1 ether)) / stSupply;
        }

        return
            (user_.stAmount * accCLMPerST) /
            (1 ether) -
            user_.finishedCLM +
            user_.pendingCLM;
    }
    ...
    function updatePool(uint256 _pid) public checkPid(_pid) {
        ...
        (bool success1, uint256 totalCLM) = getMultiplier(
            pool_.lastRewardBlock,
            block.number
        ).tryMul(pool_.poolWeight);
        require(success1, "overflow");

        (success1, totalCLM) = totalCLM.tryDiv(totalPoolWeight);
        require(success1, "overflow");

        uint256 stSupply = pool_.stTokenAmount;
        if (stSupply > 0) {
            (bool success2, uint256 totalCLM_) = totalCLM.tryMul(1 ether);
            require(success2, "overflow");

            (success2, totalCLM_) = totalCLM_.tryDiv(stSupply);
            require(success2, "overflow");

            (bool success3, uint256 accCLMPerST) = pool_.accCLMPerST.tryAdd(
                totalCLM_
            );
            require(success3, "overflow");
            pool_.accCLMPerST = accCLMPerST;
        }

        pool_.lastRewardBlock = block.number;

        emit UpdatePool(_pid, pool_.lastRewardBlock, totalCLM);
    }
    ...
    /**
     * @notice Deposit staking ETH for CLM rewards
     */
    ...
    /**
     * @notice Deposit staking token for CLM rewards
        ...
        whenNotWithdrawPaused
    {
        ...
        uint256 pendingCLM_ = (user_.stAmount * pool_.accCLMPerST) /
            (1 ether) -
            user_.finishedCLM;

        if (pendingCLM_ > 0) {
            user_.pendingCLM = user_.pendingCLM + pendingCLM_;
        }
        ...
        user_.finishedCLM = (user_.stAmount * pool_.accCLMPerST) / (1 ether);
    ...
    /**
     * @notice Claim CLM tokens reward
     *
        ...
        whenNotClaimPaused
    {
        ...
        if (pendingCLM_ > 0) {
            user_.pendingCLM = 0;
            _safeCLMTransfer(msg.sender, pendingCLM_);
        }

        user_.finishedCLM = (user_.stAmount * pool_.accCLMPerST) / (1 ether);

        emit Claim(msg.sender, _pid, pendingCLM_);
    }
    ...
    function _deposit(uint256 _pid, uint256 _amount) internal {
        ...
        if (user_.stAmount > 0) {
            // uint256 accST = user_.stAmount.mulDiv(pool_.accCLMPerST, 1 ether);
            (bool success1, uint256 accST) = user_.stAmount.tryMul(
                pool_.accCLMPerST
            );
            require(success1, "user stAmount mul accCLMPerST overflow");
            (success1, accST) = accST.tryDiv(1 ether);
            require(success1, "accST div 1 ether overflow");

            (bool success2, uint256 pendingCLM_) = accST.trySub(
                user_.finishedCLM
            );
            require(success2, "accST sub finishedCLM overflow");

            if (pendingCLM_ > 0) {
                (bool success3, uint256 _pendingCLM) = user_.pendingCLM.tryAdd(
                    pendingCLM_
                );
                require(success3, "user pendingCLM overflow");
                user_.pendingCLM = _pendingCLM;
            }
        }
        ...
        // user_.finishedCLM = user_.stAmount.mulDiv(pool_.accCLMPerST, 1 ether);
        (bool success6, uint256 finishedCLM) = user_.stAmount.tryMul(
            pool_.accCLMPerST
        );
        require(success6, "user stAmount mul accCLMPerST overflow");

        (success6, finishedCLM) = finishedCLM.tryDiv(1 ether);
        require(success6, "finishedCLM div 1 ether overflow");

        user_.finishedCLM = finishedCLM;

        emit Deposit(msg.sender, _pid, _amount);
    }

    /**
     * @notice Safe CLM transfer function, just in case if rounding error causes pool to not have enough CLMs
     *
     * @param _to        Address to get transferred CLMs
     * @param _amount    Amount of CLM to be transferred
     */
    function _safeCLMTransfer(address _to, uint256 _amount) internal {
        uint256 CLMBal = CLM.balanceOf(address(this));

        if (_amount > CLMBal) {
            CLM.transfer(_to, CLMBal);
        } else {
            CLM.transfer(_to, _amount);
        }
    }
